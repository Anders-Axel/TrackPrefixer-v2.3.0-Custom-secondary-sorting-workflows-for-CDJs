...
Continues from the highest matching prefix in the configured target field.
The library is scanned batch by batch. Only primitive counters are retained
between asynchronous batch calls. Selected tracks are sorted through integer
indexes using the same saved primary and secondary rules as Number selected tracks.
*/
...
let highestLibraryPrefix = 0
let highestLibraryPrefixWidth = 1
let tracksScanned = 0
let incomingTracksIgnored = 0
let mainLibraryTrackCount = 0
let prefixedTracksFound = 0
...
while (batch.length > 0) {
  for (let batchIndex = 0; batchIndex < batch.length; batchIndex++) {
    ...
    if (incoming) {
      ...
      const currentValue = String(batch[batchIndex][targetField] || "")
      const prefixMatch = currentValue.match(/^([0-9]+)\*\s/)

      if (prefixMatch) {
        const prefixNumber = Number(prefixMatch[1])

        if (!isNaN(prefixNumber) && prefixNumber >= 0) {
          prefixedTracksFound = prefixedTracksFound + 1

          if (prefixNumber > highestLibraryPrefix) {
            highestLibraryPrefix = prefixNumber
            highestLibraryPrefixWidth = prefixMatch[1].length
          } else if (prefixNumber === highestLibraryPrefix && prefixMatch[1].length > highestLibraryPrefixWidth) {
            highestLibraryPrefixWidth = prefixMatch[1].length
          }
        }
      }
...
for (let selectedIndex = 0; selectedIndex < selectedTracks.length; selectedIndex++) {
  const selectedValue = String(selectedTracks[selectedIndex][targetField] || "")
  const selectedPrefixMatch = selectedValue.match(/^[0-9]+\*\s/)

  if (!(selectedPrefixMatch && existingPrefixChoice === "Skip")) {
    plannedChanges = plannedChanges + 1
  }
}

const plannedLastNumber = highestLibraryPrefix + plannedChanges
let prefixWidth = highestLibraryPrefixWidth

if (prefixedTracksFound === 0) {
  prefixWidth = 1
}
...
let nextNumber = highestLibraryPrefix + 1
let changed = 0
let skippedAlreadyNumbered = 0
let replacedExistingPrefixes = 0
let firstAssignedNumber = 0
...
for (let orderIndex = 0; orderIndex < selectedIndexes.length; orderIndex++) {
  ...
  if (prefixMatch && existingPrefixChoice === "Skip") {
    skippedAlreadyNumbered = skippedAlreadyNumbered + 1
  } else {
    let cleanValue = currentValue

    if (prefixMatch) {
      cleanValue = currentValue.substring(prefixMatch[0].length)
      replacedExistingPrefixes = replacedExistingPrefixes + 1
    }

    if (firstAssignedNumber === 0) {
      firstAssignedNumber = nextNumber
    }
...
let lastAssignedNumber = highestLibraryPrefix
...
const filename = "continue-numbering-v2.1-" + timestamp + ".txt"
const log =
  "TrackPrefixer run log\n" +
  "Action: Continue selected numbering v2.1\n" +
  ...
  "Prefix width mode: ignored by Continue\n" +
  "Highest prefix width found: " + highestLibraryPrefixWidth + "\n" +
  ...
  "Prefixed library tracks found: " + prefixedTracksFound + "\n" +
  "Highest library prefix: " + highestLibraryPrefix + "\n" +
  "Selected tracks: " + selectedTracks.length + "\n" +
  "Tracks numbered: " + changed + "\n" +
  "Existing prefixes replaced: " + replacedExistingPrefixes + "\n" +
  "Existing prefixes skipped: " + skippedAlreadyNumbered + "\n" +
  "Unparsed Date Added values: " + unparsedDateAddedValues + "\n" +
  "First assigned number: " + firstAssignedNumber + "\n" +
...
_helpers.Report("Library tracks scanned: " + tracksScanned)
_helpers.Report("Incoming tracks ignored: " + incomingTracksIgnored)
_helpers.Report("Highest library prefix: " + highestLibraryPrefix)
_helpers.Report("Highest prefix width found: " + highestLibraryPrefixWidth)
_helpers.Report("Prefix width used: " + prefixWidth)
_helpers.Report("Tracks numbered: " + changed)
_helpers.Report("Existing prefixes replaced: " + replacedExistingPrefixes)
_helpers.Report("Existing prefixes skipped: " + skippedAlreadyNumbered)
